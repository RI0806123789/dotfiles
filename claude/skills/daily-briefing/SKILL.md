...
2. 居住地は<CITY>、KIT（金沢工業大学）もその近辺のため、カレンダーのタイムゾーンは常に `Asia/Tokyo`、天気検索の地点も特に指定がなければ<CITY>を使う。
...
3. **天気**: `WebSearch`で`"<CITY> 天気 今日 <current date>"`を検索。最高/最低気温・降水確率・天候を取得し、服装を提案する。
...
3. **天気**: `WebSearch`で`"<CITY> 天気 明日 <翌日の日付>"`を検索し、朝モードと同じ基準（時間帯別の推移も含む）でフルの服装提案を行う。
